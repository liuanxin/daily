...
# gc 日志
gc_file="${log}/gc-${project}-${profile}-%t.log"
...
JAVA_OPTS="$JAVA_OPTS -verbose:gc -XX:+PrintGCDetails -XX:+PrintGCDateStamps -XX:+PrintGCTimeStamps -XX:GCLogFileSize=20M -Xloggc:${gc_file}"
...
jvm_args="${jvm_args} -verbose:gc -XX:+PrintGCDetails -XX:+PrintGCDateStamps -XX:+PrintGCTimeStamps -XX:GCLogFileSize=20M"
...
release_jar() {
    ...
    start_jar "${current_release}" "gc-${package##*/}-${profile}-%t.log" "${debug_port}"
    log "「${comment}」发布完成"
}
# 还原 jar 包项目
revert_jar() {
...
release_jar() {
    ...
    package="$2"
    file_name="$3"
    gc_file="gc-${package}-%{profile}-%t.log"
