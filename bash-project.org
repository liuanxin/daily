...
update() {
    ...
    # Git >= 2.22 开始可以用 git branch --show-current 命令
    log "当前分支 --> $(git rev-parse --abbrev-ref HEAD)"
    # Git >= v2.6.0-rc0 开始可以加 --date 选项 --date=format:'%Y-%m-%d %H:%M:%S'
...
release() {
    ...
    cd ${publish}/${project}
    eval "$(readlink -f /bin/java) ${jvm_args} -Dspring.config.location=${publish}/application.yml -Xloggc:${log_path}/gc-${project}-$(date +%F).log -jar ${publish}/${project}/${project}.jar &"
    sleep 5
