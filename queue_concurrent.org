...
java 7 以后, ConcurrentLinkedQueue、公平模式的 SynchronousQueue(默认构造)、无界的 LinkedBlockingQueue 可以用 LinkedTransferQueue 替代
#+END_EXAMPLE
...
private Node tryAppend(Node s, boolean haveData) {
 ...
        // CAS 失败, 一般来说失败的原因在于 p.next != null, 可能有其他增加了 tail, 向前推进
...
private E awaitMatch(Node s, Node pred, E e, boolean timed, long nanos) {
    ...
    for (;;) {
        ...
        // 线程中断或者超时了. 则调用将 s 节点 item 设置为 e, 等待取消
