...
Explain: mysql 使用 rr 做为默认隔离级别的主要原因是在于 binlog.
在 5.1 之前, 基于语句(statement) 是 binlog 的默认格式, 之后就有了 row(行) 和 mixed(混用) 两种模式.
从 5.1 开始, 如果使用 基于语句(statement) 的 binlog(以 commit 为序), 将不支持 rc 和 ru 级别(不支持 rc 级别的原因是它的执行顺序是不正确的).
...
commit;
#+END_SRC
在 5.1 及以上的 rc 级别下, 基于语句(statement)的 binlog 执行结果是不正确的.
...
commit;                       -- 3
#+END_SRC
与 rc 级别不同的是, 在 rr 中由于要保证可重复读, 会话二的 delete 将会被会话一阻塞, 提交会话一提交.
在 rr 中, 会话一的语句会先在 t1 的记录上 共享 s 锁(5.1 的 rc 不会, 5.0 的 rc 会), 由于 会话一没有提交, 会话二的 delte 需要等待于是阻塞.

5.0 的 rc 与 5.1 的 rr 使用的类似的并发和上锁机制, 也就是说, 5.0 的 rc 和 5.1 及以上的 rc 会有兼容问题.
