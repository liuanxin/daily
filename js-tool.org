...
/**
 * 将请求接口 promise 化
 * 比如之前调用 request({ url: '/user-info', method: 'GET', data: { id: 123 } })
 * 现在可以改成 promisify(request, { url: '/user-info', method: 'GET', data: { id: 123 } }).then((info) => { ... }).catch((error) => { ... })
 * 或者可以使用 const returnInfo = await promisify(request, { url: '/user-info', method: 'GET', data: { id: 123 } })
 */
